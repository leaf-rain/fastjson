[![Build Status](https://travis-ci.org/leaf-rain/fastjson.svg)](https://travis-ci.org/leaf-rain/fastjson)
[![GoDoc](https://godoc.org/github.com/leaf-rain/fastjson?status.svg)](http://godoc.org/github.com/leaf-rain/fastjson)
[![Go Report](https://goreportcard.com/badge/github.com/leaf-rain/fastjson)](https://goreportcard.com/report/github.com/leaf-rain/fastjson)
[![codecov](https://codecov.io/gh/leaf-rain/fastjson/branch/master/graph/badge.svg)](https://codecov.io/gh/leaf-rain/fastjson)
...
## Features
  ...
  * Provides simple [API](http://godoc.org/github.com/leaf-rain/fastjson).
  ...
  * May quickly extract a part of the original JSON with `Value.Get(...).MarshalTo` and modify it
    with [Del](https://godoc.org/github.com/leaf-rain/fastjson#Value.Del)
    and [Set](https://godoc.org/github.com/leaf-rain/fastjson#Value.Set) functions.
  * May parse array containing values with distinct types (aka non-homogenous types).
    For instance, `fastjson` easily parses the following JSON array `[123, "foo", [456], {"k": "v"}, null]`.
  * `fastjson` preserves the original order of object items when calling
    [Object.Visit](https://godoc.org/github.com/leaf-rain/fastjson#Object.Visit).


## Known limitations

  * Requies extra care to work with - references to certain objects recursively
    returned by [Parser](https://godoc.org/github.com/leaf-rain/fastjson#Parser)
    must be released before the next call to [Parse](https://godoc.org/github.com/leaf-rain/fastjson#Parser.Parse).
    Otherwise the program may work improperly. The same applies to objects returned by [Arena](https://godoc.org/github.com/leaf-rain/fastjson#Arena).
    Adhere recommendations from [docs](https://godoc.org/github.com/leaf-rain/fastjson).
  * Cannot parse JSON from `io.Reader`. There is [Scanner](https://godoc.org/github.com/leaf-rain/fastjson#Scanner)
...
See also [examples](https://godoc.org/github.com/leaf-rain/fastjson#pkg-examples).
...
## Performance optimization tips
  ...
  * Re-use [Parser](https://godoc.org/github.com/leaf-rain/fastjson#Parser) and [Scanner](https://godoc.org/github.com/leaf-rain/fastjson#Scanner)
    for parsing many JSONs. This reduces memory allocations overhead.
    [ParserPool](https://godoc.org/github.com/leaf-rain/fastjson#ParserPool) may be useful in this case.
  * Prefer calling `Value.Get*` on the value returned from [Parser](https://godoc.org/github.com/leaf-rain/fastjson#Parser)
  ...
  * Prefer calling once [Value.Get](https://godoc.org/github.com/leaf-rain/fastjson#Value.Get)
    for common prefix paths and then calling `Value.Get*` on the returned value
    for distinct suffix paths.
  * Prefer iterating over array returned from [Value.GetArray](https://godoc.org/github.com/leaf-rain/fastjson#Object.Visit)
...
go-fuzz-build github.com/leaf-rain/fastjson
...
```
$ GOMAXPROCS=1 go test github.com/leaf-rain/fastjson -bench='Parse$'
goos: linux
goarch: amd64
pkg: github.com/leaf-rain/fastjson
...
```
$ GOMAXPROCS=1 go test github.com/leaf-rain/fastjson -bench='Validate$'
goos: linux
goarch: amd64
pkg: github.com/leaf-rain/fastjson
...
## FAQ
  ...
  * Q: _There are a ton of other high-perf packages for JSON parsing in Go. Why creating yet another package?_
    A: Because other packages require either rigid JSON schema via struct magic
       ...
       Additionally, `fastjson` provides nicer [API](http://godoc.org/github.com/leaf-rain/fastjson).
  ...
  * Q: _Why fastjson doesn't provide fast marshaling (serialization)?_
    A: Actually it provides some sort of marshaling - see [Value.MarshalTo](https://godoc.org/github.com/leaf-rain/fastjson#Value.MarshalTo).
       But I'd recommend using [quicktemplate](https://github.com/leaf-rain/quicktemplate#use-cases)
  ...
  * Q: _`fastjson` crashes my program!_
    A: There is high probability of improper use.
       * Make sure you don't hold references to objects recursively returned by `Parser` / `Scanner`
         ...
         if such restriction is mentioned in [docs](https://github.com/leaf-rain/fastjson/issues/new).
       ...
       * If your program continue crashing after fixing issues mentioned above, [file a bug](https://github.com/leaf-rain/fastjson/issues/new).
